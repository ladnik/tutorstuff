...
\begin{frame}[c, plain, fragile]
	\begin{figure}
		\centering
		\includegraphics[width=0.5\linewidth]{w13_sonic.png}
		
		{\centering\scriptsize Danke an Bjarne Hansen für dieses treffende Bild}
	\end{figure}
\end{frame}

\begin{frame}<2->[c, fragile]{BDDs: ITE-Algorithmus}
	\tikzset{
		highlight/.style={draw=TUMBlue, thick, fill=TUMBlue, fill opacity=0.35, text opacity=1},
		ahighlight/.style={draw=TUMBlue, thick},
		highlight on/.style={alt=#1{highlight}{}},
	...
	\only<2-17>{
		\begin{center}
			\begin{tikzpicture}
				...
				\node[left=\labelspacing of r1, visible on=<3->] {$A_1 \oplus B_1$};

				\node[c] (r2l) [below=\smallspace of r1, xshift=-2*\smallspace, visible on=<4->, highlight on=<{4,7,10}>] {$b$};
				\node[left=\labelspacing of r2l, visible on=<4->] {$A_2 \oplus B_3$};
				...
				\node[below=\labelspacing of r2r, visible on=<12->] {$A_3 \oplus B_2$};
