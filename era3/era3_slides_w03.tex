...
\begin{frame}[c, fragile]{Speicherzugriffe\footnote{\textbf{b}yte: 1 Byte, \textbf{h}alf-word: 2 Byte, \textbf{w}ord: 4 Byte, Suffix \textbf{u}nsigned}}{}
	\begin{columns}[c]
		\begin{column}{0.5\textwidth}
			\begin{center}
				\only<1-3>{
				...
				\only<4-6>{
					...
					\vspace{\baselineskip}
					lade 1 Byte (sign-extended) an der Adresse \texttt{a2} \textminus{} 4 Bytes Offset in das Register \texttt{t1}
				}
				...
				\begin{figure}
					\begin{tikzpicture}
						...
						\only<1-3>{
							...
							\node[biglabel, below=1cm of mat] {t0 = 0x04030201};
						}
						...
						\only<4-6>{
							...
							\node[biglabel, below=1cm of mat] {t1 = 0xffffffaa};
						}
