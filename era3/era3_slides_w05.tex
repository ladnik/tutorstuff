...
\begin{frame}[t]{Terminologie}
	...
	\begin{itemize}
		...
		\item \textbf{räumliche} Lokalität: Zugriff auf $x$ $\rightarrow$ Zugriff auf Daten in der Nähe wahrscheinlicher\\$\rightarrow$ Konsequenz: Cachen von ganzer Cacheline
	\end{itemize}
\end{frame}
		...
		\begin{column}{0.5\textwidth}
			\only<1-3>{
				\begin{itemize}
					...
					\item jedes Datum aus dem Hauptspeicher kann nur an \textit{einer} bestimmten Stelle im Cache gecached werden
			...
			\only<4->{
				\begin{center}
					...
					\scriptsize Es wird immer die gesamte Cacheline ausgetauscht!
				\end{center}
			}
		\end{column}%
		\begin{column}{0.5\textwidth}
		...
		\begin{column}{0.5\textwidth}
			\only<1-3>{
				\begin{itemize}
					...
					\item jedes Datum aus dem Hauptspeicher kann an einer \textit{beliebigen} Stelle im Cache gecached werden
					\item <2->Beispiel: 16-Bit-Architektur, 8 Cachezeilen zu je 4 Byte
					      \begin{enumerate}
						      \item Anzahl Offset-Bits:  \visible<3->{$\log_2(4)=2$}
						      \item Anzahl Index-Bits: \visible<3->{$0$}
						      \item Anzahl Tag-Bits: \visible<3->{$16-3-0=13$}
					      \end{enumerate}
				\end{itemize}
			}
			\only<4->{
				\begin{center}
					\resizebox{\textwidth}{!}{
						\begin{tikzpicture}
							...
							\node[below=0cm of mat-1-1.south, anchor=north] {\textrm{13 bit}};
							...
							\visible<5->{
								...
								\node[below=0cm of mat2-1-1.south, anchor=north] {\textrm{13 bit}};
		...
		\begin{column}{0.5\textwidth}
			\only<1-3>{
				\begin{itemize}
					...
					\item jedes Datum aus dem Hauptspeicher kann nur in \textit{einem} bestimmtem Set gecached werden, innerhalb des Sets aber an \textit{beliebiger} Stelle stehen
