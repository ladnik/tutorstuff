...
\title{Übung 14: SAT und \\Physical Design}
...
\begin{frame}[c]{SAT}{}
	\begin{columns}[c]
		\begin{column}{0.8\textwidth}
			\begin{itemize}
				\item \enquote{Satisfiability}, d.h. Erfüllbarkeit einer boolschen Funktion herausfinden:
				      \begin{itemize}
					      \item  \enquote{Existiert eine Belegung $x_0, \ldots, x_n$ sodass $f(x_0, \ldots, x_n)\equiv 1$?}
					      \item \enquote{Gibt es in der Wahrheitstabelle mindestens eine Zeile  mit Ergebnis $1$?}
				      \end{itemize}
				\item moderne Solver können sichere Aussage über Erfüllbarkeit bzw. Nicht-Erfüllbarkeit treffen, ohne alle Variablenbelegungen durchzuprobieren
				\item Übertragung verschiedenster Probleme auf SAT, Formulierung als KNF\footnote[frame]{Konjunktive Normalform, CNF}: OR in den Klauseln, AND zwischen Klauseln
			\end{itemize}
		\end{column}%
...
\begin{frame}[c, fragile]{Tseitin-Transformation}{}
	\begin{columns}[c]
		\begin{column}{0.5\textwidth}
			\begin{center}
				\begin{circuitikz}
		...
		\begin{column}{0.5\textwidth}
			\begin{align*}
				...
				\circled{4} \;\; & e
			\end{align*}
			nach Umformung zu KNF und Berechnung mittels eines SAT-Solvers erhalten wir \texttt{UNSAT}, die Schaltkreise sind also äquivalent\footnote[frame]{mittels De-Morgan einfach nachprüfbar: $\n{a\cdot b} \equiv \n{a}+\n{b}$}
		\end{column}
	\end{columns}
				...
				\resizebox{0.8\textwidth}{!}{
					\begin{tikzpicture}
						...
						\only<13>{
							...
							\draw[bboxline] (3,2) -- (3,5);
						}

						\only<14, 15>{
							\draw[steinerbranch] (5,2) -- (3,2);
							\draw[steinerbranch] (3,2) -- (3,5);
						}
		...
		\begin{column}{0.5\textwidth}
			...
			\only<3->{
				...
				\begin{enumerate}
					...
					\item<5-> Wähle die Verbindung, auf der der Hanan-Punkt mit geringstem Abstand zu einem der anderen Terminalknoten liegt
					\item<6-> Füge diese Verbindung dem Steinerbaum hinzu und fahre mit Schritt 2 fort, ausgehend vom Hanan-Punkt mit geringstem Abstand
				\end{enumerate}
